...
import { ColProps, Form, FormSchema, FormSetProps } from '@/components/Form'
...
const props = defineProps({
  ...
  // 固定每行展示几列，设置后优先于响应式配置
  columnCount: propTypes.number.validate((v: number) => v > 0 && v <= 24 && 24 % v === 0).def(0),
  // 是否自动设置搜索项响应式栅格
  autoResponsive: propTypes.bool.def(true),
  // 搜索项响应式栅格配置
  responsiveColProps: {
...
const newSchema = computed(() => {
  ...
  if (propsComputed.isCol && propsComputed.columnCount) {
    schema = schema.map((item) => ({
      ...item,
      colProps: {
        span: 24 / propsComputed.columnCount!,
        ...item.colProps
      }
    }))
  } else if (propsComputed.autoResponsive && propsComputed.isCol) {
    schema = schema.map((item) => ({
      ...item,
      colProps: {
        ...propsComputed.responsiveColProps,
        ...item.colProps
      }
    }))
  }
